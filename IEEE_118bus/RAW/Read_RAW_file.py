...
with open('case118_modify_genafter_FS_2.raw', 'r') as csvfile:
...
#    # -------------------------------------------------------------------------
    ...
    for row in reader:
        ...
        if  (idx >= load_row[0]) and (idx <= load_row[1]): 
           ...
           load_ID          = load_ID.replace("'", "")           
           if (load_ID != 'PV') and (load_ID != 'SH'): # if this is a load
               identifier   = 'load_Z_' + str(bus_number) + '_' + load_ID
               PLoadList    += [identifier + '/P']
               QLoadList    += [identifier + '/Q']
           elif load_ID == 'PV': # if this is a PV -> model it as a negative load with differnet ID
               identifier   = 'load_Z_' + str(bus_number) + '_' + load_ID
               PpvList      += [identifier + '/P']
               QpvList      += [identifier + '/Q']               
           elif load_ID == 'SH': # if this is a PV -> model it as a negative load with differnet ID
               identifier   = 'load_Z_' + str(bus_number) + '_' + load_ID
        ...
        if (idx >= gen_row[0]) and (idx <= gen_row[1]):
           ...
           PGenList         += [identifier + '/Pe']
           QGenList         += [identifier + '/Qe']
...
#           shuntList        += [identifier + '/Q']  
    ...
    # Ppv rows
    PpvList                 = ['incoming', 'P_pv'] + PpvList     
    ws_Pins.append(PpvList)    
    # Pgen rows
    PGenList                = ['incoming', 'P_Gen'] + PGenList     
    ...
    """ Control variables """  
    # Qpv rows
    QpvList                 = ['incoming', 'Q_pv'] + QpvList
    ws_Pins.append(QpvList)
    # Qpv rows
    shuntList                 = ['incoming', 'shunt'] + shuntList
    ws_Pins.append(shuntList)    
    # Qgen rows  ->>> NOT NEEDED !
    QGenList                = ['incoming', 'Q_Gen'] + QGenList     
    ws_Pins.append(QGenList)      
